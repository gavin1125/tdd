...
class OptionParsers {
    ...
    public static <T> OptionParser<T[]> list(IntFunction<T[]> generator, Function<String, T> valueParser) {
        return (arguments, option) -> values(arguments, option).map(it -> it.stream().map(value -> parseValue(option, value, valueParser)).toArray(generator)).orElse(generator.apply(0));
    }
    ...
    private static Optional<List<String>> values(List<String> arguments, Option option, int expectSize) {
        return values(arguments, option).map(it -> checkSize(option, expectSize, it));
    }

    private static List<String> checkSize(Option option, int expectSize, List<String> values) {
        if (values.size() < expectSize) throw new InsufficientArgumentsException(option.value());
        if (values.size() > expectSize) throw new TooManyArgumentsException(option.value());
        return values;
    }

    private static List<String> values(List<String> arguments, int index) {
        return arguments.subList(index + 1, IntStream.range(index + 1, arguments.size()).filter(it -> arguments.get(it).matches("^-[a-zA-Z-]+$")).findFirst().orElse(arguments.size()));
    }
